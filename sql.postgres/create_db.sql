...
CREATE VIEW crawl_performance_timestampe AS
    SELECT timestamp,
        sum(frontier_received) as frontier_received,
        sum(frontier_processed) as frontier_processed,
        --sum(frontier_disappear) as frontier_disappear,
...
BEGIN
    ...
    -- updating the table
    DELETE FROM crawl_performance WHERE timestamp > (SELECT max(timestamp)-interval '5 minutes' FROM crawl_performance);
    INSERT INTO crawl_performance 
        ( SELECT * 
            FROM crawl_performance_view
            WHERE timestamp > (SELECT max(timestamp)-interval '5 minutes' FROM crawl_performance)
        )
        ON CONFLICT (hostname,timestamp) DO NOTHING;
END;
$$ LANGUAGE plpgsql;

CREATE VIEW crawl_performance_view AS
    SELECT 
        t1.hostname,
        t1.timestamp,
        frontier_received,
    ...
    INNER JOIN (
        SELECT 
            hostname,date_trunc('minute', frontier.timestamp_processed) as timestamp,
            count(1) as frontier_disappear
        FROM frontier
        INNER JOIN urls on urls.id_urls=frontier.id_urls
        WHERE
            frontier.timestamp_processed is not null and
            frontier.id_frontier not in (select id_frontier from responses)
        GROUP BY hostname,timestamp
    ) AS t3 ON t1.timestamp=t3.timestamp and t1.hostname=t3.hostname
    */
    FULL OUTER JOIN (
        SELECT 
    ...
    FULL OUTER JOIN (
        ...
        INNER JOIN urls on urls.id_urls=frontier.id_urls
        GROUP BY hostname,timestamp
    ) AS t7 ON t1.timestamp=t7.timestamp and t1.hostname=t7.hostname
    FULL OUTER JOIN (
        SELECT 
            hostname,date_trunc('minute', responses.timestamp_processed) as timestamp,
...
CREATE VIEW crawl_perf_summary AS
    SELECT timestamp,
        sum(responses_processed) as responses_processed,
        sum(bytes) as bytes
    FROM crawl_perf
...
CREATE FUNCTION crawl_perf_update()
RETURNS void AS $$
BEGIN
    -- the most recent additions to crawl_performance are likely to be slightly
    -- out of date and need recomputing; therefore we delete them before
    -- updating the table
    --DELETE FROM crawl_performance WHERE timestamp > (SELECT max(timestamp)-interval '20 minutes' FROM crawl_performance);
    INSERT INTO crawl_perf (SELECT * FROM crawl_perf_view)
        ON CONFLICT (hostname,timestamp) DO NOTHING;
END;
$$ LANGUAGE plpgsql;

CREATE VIEW crawl_perf_view AS
    SELECT 
        hostname,
        timestamp,
        responses_processed,
    ...
    FROM (
        ...
        INNER JOIN frontier on frontier.id_frontier=responses.id_frontier
        INNER JOIN urls on urls.id_urls=frontier.id_urls
        WHERE responses.timestamp_processed is not null
        GROUP BY hostname,timestamp
    ) AS t5 
...
CREATE VIEW crawl_perf2_summary AS
    SELECT timestamp,
        sum(responses_processed) as responses_processed,
        sum(bytes) as bytes
    FROM crawl_perf2
...
CREATE FUNCTION crawl_perf2_update()
RETURNS void AS $$
BEGIN
    -- the most recent additions to crawl_perf2ormance are likely to be slightly
    -- out of date and need recomputing; therefore we delete them before
    -- updating the table
    --DELETE FROM crawl_perf2ormance WHERE timestamp > (SELECT max(timestamp)-interval '20 minutes' FROM crawl_perf2ormance);
    INSERT INTO crawl_perf2 (SELECT * FROM crawl_perf2_view)
        ON CONFLICT (hostname,timestamp) DO NOTHING;
END;
$$ LANGUAGE plpgsql;

CREATE VIEW crawl_perf2_view AS
    SELECT 
        hostname,
        timestamp,
        responses_processed,
