...
def insert_request(connection,url,priority=0,allow_dupes=False,depth=0,flexible_url=False,update_priority=False):
    '''
    ...
    if not allow_dupes:
        if update_priority:
            sql=sqlalchemy.sql.text('''select priority from frontier where id_urls=:id_urls limit 1;''')
        else:
            sql=sqlalchemy.sql.text('''select 1 from frontier where id_urls=:id_urls limit 1;''')
        ...
        if res is not None:
            if update_priority and priority>=100 and res['priority']<1000:
                sql=sqlalchemy.sql.text('''
    ...
    # even if we allow dupes,
    # we don't want to insert the url into the frontier if it already exists but is unprocessed
    else:
        sql=sqlalchemy.sql.text('''select id_frontier from frontier where id_urls=:id_urls and timestamp_processed is null;''')
        res=connection.execute(sql,{
            'id_urls':url_info['id_urls']
            }).first()
        if res is not None:
            if update_priority: 
                sql=sqlalchemy.sql.text('''
                update frontier set priority=priority+:priority where id_frontier=:id_frontier;
                ''')
                res=connection.execute(sql,{
                    'priority':priority,
                    'id_frontier':url_info['id_']
                    })
            return url_info

    # we reach this line if the checks above failed to find a reason we shouldn't insert;
